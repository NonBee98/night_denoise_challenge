    ...
    def __init__(self, in_size, reduction=4):
        ...
        self.se = nn.Sequential(
            ...
            nn.Conv2d(in_size // reduction,
                      ...
                      stride=1,
                      padding=0,
                      bias=False), nn.BatchNorm2d(in_size), nn.Hardsigmoid())
...
class MobileNetV3_Large(nn.Module):
    ...
    def __init__(self, num_classes=3, finetune=False):
        ...
        self.bneck = nn.Sequential(
            Block(5, 32, 128, 32, nn.ReLU(inplace=True), None, 1),
            Block(5, 32, 128, 32, nn.ReLU(inplace=True), SeModule(32), 1),
            Block(3, 32, 256, 64, nn.Hardswish(inplace=True), None, 2),
            Block(3, 64, 256, 64, nn.Hardswish(inplace=True), None, 1),
            Block(3, 64, 192, 64, nn.Hardswish(inplace=True), None, 1),
            Block(3, 64, 192, 64, nn.Hardswish(inplace=True), None, 1),
            Block(3, 64, 512, 128, nn.Hardswish(inplace=True), SeModule(128),
                  1),
            Block(3, 128, 640, 128, nn.Hardswish(inplace=True), SeModule(128),
                  1),
            Block(5, 128, 640, 128, nn.Hardswish(inplace=True), SeModule(128),
                  1),
            Block(5, 128, 640, 160, nn.Hardswish(inplace=True), SeModule(160),
                  2),
            Block(5, 160, 960, 160, nn.Hardswish(inplace=True), SeModule(160),
                  1),
        )
...
class MobileNetV3_Small(nn.Module):
    ...
    def __init__(self, num_classes=1000):
        ...
        self.hs2 = nn.Hardswish(inplace=True)
        self.linear3 = nn.Linear(576, 640)
        self.bn3 = nn.BatchNorm1d(640)
        self.hs3 = nn.Hardswish(inplace=True)
        self.linear4 = nn.Linear(640, num_classes)
        self.init_params()
